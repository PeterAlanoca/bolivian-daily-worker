...
public class NewsSourceParserRegistry : INewsSourceParserRegistry
{
    private readonly IReadOnlyDictionary<string, INewsSourceParser> _parsers;

    public NewsSourceParserRegistry(IEnumerable<INewsSourceParser> parsers)
    {
        _parsers = parsers.ToDictionary(parser => parser.SourceAlias, StringComparer.OrdinalIgnoreCase);
    }
